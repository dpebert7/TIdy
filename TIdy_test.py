...
def test_keyword_capitalization():
    ...
    assert if_TI.outtext == ['IF(1=0)']


def test_indentation():
    indent_TI.indent()
    assert indent_TI.outtext == ['IF(1=0);', DEFAULT_TAB + "ASCIIOutput('zz.txt', 'Hello world!');", 'ENDIF;']


def test_trailing_whitespace():
    trailing_whitespace_TI.remove_trailing_whitespace()
    assert trailing_whitespace_TI.outtext == ['#Comment']


def test_capitalization_context():
    comment_TI.tidy()
    assert comment_TI.outtext != ['### ConsTANts']